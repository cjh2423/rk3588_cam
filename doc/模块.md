...
- **ConfigManager**: 单例配置管理器，支持配置文件的加载/保存与运行时参数修改。
...
### 1.2 GUI 层 (交互界面)
- **CameraView**: 主界面，显示实时画面与识别结果，支持考勤状态弹窗提示。
- **UserManagerWidget**: 用户管理对话框，支持查看与删除用户。
- **AttendanceManagerWidget**: 考勤管理对话框，支持记录查询、搜索、导出与删除。
- **SettingsDialog**: 系统设置对话框，用于修改阈值、考勤规则等。
...
- **AttendanceService**: 考勤业务逻辑封装，包含打卡防抖、迟到/早退判定。
- **FeatureLibrary**: 内存特征库，支持 1:N 高速搜索。
- **DatabaseManager**: SQLite 连接管理。
- **DAO**: User/FaceFeature/Attendance 数据访问对象，支持多表联查。
